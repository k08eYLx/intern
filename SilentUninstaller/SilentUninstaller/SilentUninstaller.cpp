...
BEGIN_MESSAGE_MAP(CSilentUninstallerApp, CWinApp)
...
CSilentUninstallerApp::CSilentUninstallerApp()
{
...
CSilentUninstallerApp theApp;


// CSilentInstallerApp initialization

BOOL CSilentUninstallerApp::InitInstance()
{
	//* 创建互斥体，让程序只能打开一次
	HANDLE hMutex = CreateMutex(NULL, TRUE, _T("SilentUninstallerMutex"));
	if (GetLastError() == ERROR_ALREADY_EXISTS) {
		...
		// 安装程序各个页面的title
#ifdef DEBUG
		CString titles[] = { "Welcome", "Confirm", "Finish" };
#else
		CString titles[] = { "Confirm", "Finish" };
#endif
	...
	SilentUninstallerDlg mainDialog;
	mainDialog.DoModal();
